...
import { Utensils, Coffee, Clock, MapPin, Navigation, Search, Plus, Trash2, Edit2, Save, X, Star, ArrowLeft } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row justify-between items-center py-2 sm:py-4 gap-2 sm:gap-4">
            <div className="flex items-center gap-3 sm:gap-4 w-full md:w-auto overflow-hidden">
              <Link to="/" className="bg-slate-100 hover:bg-slate-200 text-slate-700 p-1.5 sm:p-2.5 rounded-full transition-all flex-shrink-0">
                <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5" />
              </Link>
              <h2 className="text-sm sm:text-2xl font-bold text-slate-900 truncate">
                {t(`সিলেটের ${restaurants.length}টি রেস্টুরেন্ট`, `Top ${restaurants.length} Restaurants`)}
              </h2>
            </div>
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
              <div className="relative flex-grow md:w-64 lg:w-72">
                ...
                <input
                  ...
                  className="block w-full pl-7 sm:pl-9 pr-3 py-1.5 sm:py-2 border border-slate-300 rounded-lg sm:rounded-xl leading-5 bg-slate-50 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 text-xs sm:text-sm transition-colors"
                  value={searchTerm}
              ...
              {editMode && (
                <div className="flex gap-1 sm:gap-2 flex-shrink-0">
                  <button
                    ...
                    className="bg-green-600 text-white p-1.5 sm:p-2 rounded-lg sm:rounded-xl hover:bg-green-700 transition-colors"
                  ...
                  <button
                    ...
                    className="bg-amber-500 text-white p-1.5 sm:p-2 rounded-lg sm:rounded-xl hover:bg-amber-600 transition-colors text-[10px] sm:text-sm font-bold px-2 sm:px-3"
                  >
                    Restore
                  </button>
                </div>
            ...
            {filteredRestaurants.map((restaurant, index) => (
              <div key={index} className="bg-white rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow flex flex-col h-full relative group overflow-hidden">
                ...
                <div className="p-3 sm:p-6 flex-grow flex flex-col">
                  <Link to={`/restaurant/${restaurant.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '')}`} className="flex-grow block group-hover:text-red-600 transition-colors">
                    <div className="flex flex-col sm:flex-row justify-between items-start mb-1 sm:mb-2 pr-1 sm:pr-12">
                      <h3 className="text-xs sm:text-lg font-bold text-slate-900 leading-tight w-full group-hover:text-red-600 transition-colors">{restaurant.name}</h3>
                    ...
                    <p className="text-[9px] sm:text-sm text-slate-500 mb-2 sm:mb-4">{restaurant.type}</p>
                  ...
                  <a 
                    ...
                    className="w-full bg-slate-50 hover:bg-red-50 text-slate-700 hover:text-red-700 border border-slate-200 hover:border-red-200 font-medium py-1.5 sm:py-2 px-2 sm:px-4 rounded-lg sm:rounded-xl transition-colors flex items-center justify-center text-[10px] sm:text-sm"
                  >
