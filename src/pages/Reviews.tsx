    ...
    <div className="min-h-screen bg-slate-50 pb-16">
      ...
      <div className="sticky top-0 sm:top-14 z-40 bg-white border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-2 sm:py-4 gap-4">
            ...
            <div className="flex items-center gap-2">
              <button 
                onClick={copyLink}
                className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg transition-colors text-xs sm:text-sm font-medium"
              >
                {copied ? <Check className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-green-600" /> : <LinkIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-500" />}
                <span className="hidden xs:inline">{copied ? t('কপি হয়েছে', 'Copied') : t('লিঙ্ক', 'Link')}</span>
              </button>
            </div>
          </div>
        </div>
